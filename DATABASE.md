...
- 2026-07-02: 계정 관리(Phase B) — `supabase/accounts.sql`(login_directory.user_id 연결·삭제연쇄) + Edge Function `admin-users`(create/delete/setPassword, 호출자 1·2급 검증·2급은 3급만) + `/admin/accounts`(AccountsAdmin). **배포 필요: accounts.sql 실행 + Edge Function 배포(verify_jwt off 권장, 코드가 자체 인증).** → 배포 완료(CLI, --no-verify-jwt) + 강사 전원 계정 생성(당시 고정 임시비번 — 2026-08-20 폐기, 랜덤 발급으로 전환).
...
- 2026-07-02: 시간표 재구성 완료(관리자 편집형) — 새 단일 테이블 `schedule_classes`(teacher_id·teacher_name·subject_group·division·course·target·times[jsonb]) + RLS(공개 읽기 / 1·2급·3급본인 편집). `supabase/schedule-new.sql`. 관리 `/admin/schedule`(ScheduleAdmin: 예시 가져오기·수업 CRUD·요일/시간), 공개 `lib/content/schedule.ts`로 ScheduleView·강사상세 즉시 반영. lib/data/schedule.ts = 타입·formatScheduleDays·샘플(시드). **schedule-new.sql 실행 필요.** (기존 schedule_teachers/rows/common_notices는 미사용)
...
- 2026-08-20: **비밀번호 보안 개선** — (1) `/admin/password`(PasswordChange) 신설: 로그인한 **모든 등급이 본인 비밀번호를 직접 변경**(`supabase.auth.updateUser`, service_role·Edge Function 불필요). 변경 전 현재 비밀번호로 재인증. 대시보드에 전 등급 공통 메뉴 추가. (2) AccountsAdmin의 **고정 임시비번(`saeronedu`) 하드코딩 제거** → `randomPassword()`로 계정마다 다른 임시비번 자동 생성(혼동 문자 제외 10자, `crypto.getRandomValues`). 일괄 생성·개별 생성·재발급 모두 적용. (3) 발급 결과는 **화면에 1회만 표시**(전체 복사 버튼) — 어디에도 저장하지 않음. Edge Function(admin-users)은 변경 없음 → **재배포 불필요**. ⚠️ 기존 `saeronedu` 계정 12개는 관리자에서 **재발급 필요**, 1급(개발자) 비밀번호는 Supabase 대시보드에서 초기화.
