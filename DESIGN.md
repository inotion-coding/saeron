...
- **폰트**: `Pretendard` (CDN, 폴백 system-ui). 토큰 `--font-sans`. **전 페이지 단일 폰트로 통일**(serif 미사용).
...
- **수업 시간표(/schedule)**: 서브 히어로 → [ScheduleView](components/schedule/ScheduleView.tsx)(client). 강사 페이지와 동일한 2단 필터(1차 부 중등/고등 lg탭 · 2차 과목 전체/국어·수학·영어·사회·과학, 골드 언더라인 탭). 결과: **과목별 섹션 → 강사 카드**(강사명+과목 배지+비고, 강사명은 프로필 보유 시 `/teachers/[id]` 링크) + 해당 부의 **시간표 행 목록**([ScheduleRowList](components/schedule/ScheduleRowList.tsx), 공용): 대상 배지 · 반/과목 · 내용 · 개강 / 시계+시간(원문 보존, `break-keep`) / 비고(틸). 하단 **공통 안내** 카드. 데이터 [lib/data/schedule.ts](lib/data/schedule.ts)(강사 중심 `TeacherSchedule`/`ScheduleRow`, `teacherId`로 강사 연결, `division`=high(일반고·외고)/middle(2학기 대비반), 시간 문자열은 원문 보존). 강사 상세(/teachers/[id]) 하단에도 해당 강사 시간표를 동일 ScheduleRowList로 표시.
